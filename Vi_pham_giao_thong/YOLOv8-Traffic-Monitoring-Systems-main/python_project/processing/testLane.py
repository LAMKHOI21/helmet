...
def imageMotorViolate(frame, r1, r2, c1, c2, stt):
    ...
    save_dir = os.path.join(_BASE_DIR, 'data_xe_may_vi_pham')
    os.makedirs(save_dir, exist_ok=True)
    nameFile = str(stt) + '.jpg'
    cv2.imwrite(os.path.join(save_dir, nameFile), cropped_region)


def imageCTBViolate(frame, r1, r2, c1, c2, stt):
    # Cắt vùng từ khung hình
    cropped_region = frame[r1:r2, c1:c2]
    save_dir = os.path.join(_BASE_DIR, 'data_oto_vi_pham')
    os.makedirs(save_dir, exist_ok=True)
    nameFile = str(stt) + '.jpg'
    cv2.imwrite(os.path.join(save_dir, nameFile), cropped_region)
...
def imageViolateHelmet(frame, r1, r2, c1, c2, stt):
    # Cắt vùng từ khung hình
    cropped_region = frame[r1:r2, c1:c2]
    save_dir = os.path.join(_BASE_DIR, 'data_xe_vp_bh')
    os.makedirs(save_dir, exist_ok=True)
    nameFile = str(stt) + '.jpg'
    cv2.imwrite(os.path.join(save_dir, nameFile), cropped_region)
...
if __name__ == '__main__':
    cap = cv2.VideoCapture(os.path.join(_BASE_DIR, 'Videos', 'main.mp4'))
